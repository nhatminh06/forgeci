...
const scmDeliveryColumns = `id::text,provider,delivery_id,repository_id::text,event_type,action,COALESCE(installation_id,''),COALESCE(commit_sha,''),COALESCE(ref,''),payload_sha256,pull_request_number,status,attempt_count,next_attempt_at,last_error,received_at,processed_at`

func scanSCMDelivery(row pgx.Row) (*scm.Delivery, error) {
	var out scm.Delivery
	if err := row.Scan(&out.ID, &out.Provider, &out.DeliveryID, &out.RepositoryID, &out.EventType, &out.Action, &out.InstallationID, &out.CommitSHA, &out.Ref, &out.PayloadSHA256, &out.PullRequestNumber, &out.Status, &out.AttemptCount, &out.NextAttemptAt, &out.LastError, &out.ReceivedAt, &out.ProcessedAt); err != nil {
		return nil, err
	}
...
func (s *Store) CreateSCMDelivery(ctx context.Context, in scm.Delivery) (*scm.Delivery, error) {
	...
	out, err := scanSCMDelivery(s.pool.QueryRow(ctx, `INSERT INTO scm_deliveries(id,provider,delivery_id,repository_id,event_type,action,installation_id,commit_sha,ref,pull_request_number,payload_sha256,status,attempt_count,next_attempt_at,last_error,received_at,processed_at)
		VALUES($1,$2,$3,$4,$5,$6,NULLIF($7,''),NULLIF($8,''),NULLIF($9,''),$10,$11,$12,$13,$14,$15,$16,$17)
		ON CONFLICT(provider,delivery_id) DO NOTHING RETURNING `+scmDeliveryColumns, in.ID, in.Provider, in.DeliveryID, in.RepositoryID, in.EventType, in.Action, in.InstallationID, in.CommitSHA, in.Ref, in.PullRequestNumber, in.PayloadSHA256, in.Status, in.AttemptCount, in.NextAttemptAt, in.LastError, in.ReceivedAt, in.ProcessedAt))
	if err == nil {
		return out, nil
	}
