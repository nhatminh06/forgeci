...
mkdir -p "$tmp/server" "$tmp/runner-a" "$tmp/runner-b" "$tmp/state-a" "$tmp/state-b" "$tmp/bin" "$tmp/snapshots"
...
i=0
while :; do
  "$tmp/bin/forge-server" --execution-mode remote --listen "127.0.0.1:$api_port" --runner-listen "127.0.0.1:$runner_port" --runner-token-file "$tmp/token" --workspace "$tmp/server" --snapshot-dir "$tmp/snapshots" --database-url "postgres://postgres:forgeci@127.0.0.1:$db_port/forgeci?sslmode=disable" >"$tmp/server.log" 2>&1 & server_pid=$!
...
[ "$(docker exec "$pg" psql -U postgres -d forgeci -At -c "SELECT count(*) FROM schema_migrations WHERE version IN (3,4)")" = 2 ]
